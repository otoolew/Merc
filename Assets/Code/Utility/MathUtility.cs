...
public static class MathUtility
{
    ...
    public static bool IsPerpendicular(Transform transformComp, Vector3 targetDirection, float bias = 0.01f)
    {
        float dot = Vector3.Dot(transformComp.rotation.eulerAngles.normalized, targetDirection.normalized);
    ...
    public static bool IsParallel(Transform transformComp, Vector3 targetDirection, float bias = 0.01f)
    {
        if (Vector3.Dot(transformComp.rotation.eulerAngles.normalized, targetDirection.normalized) > 1f - bias)
        {
            return true;
        }
        return false;
    ...
    public static void IsRotationParallel(Transform transformComp, Vector2 value, float speed)
    {
        if (value.magnitude > 0.1)
        {
            Vector3 direction = (Vector3.right * value.x) + (Vector3.forward * value.y);
            Quaternion targetRotation = Quaternion.LookRotation(direction, Vector3.up);
            transformComp.rotation = Quaternion.Lerp(transformComp.rotation, targetRotation, speed * Time.deltaTime);
        }
    }
